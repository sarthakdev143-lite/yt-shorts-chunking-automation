...
def process_project_task(project_id: str, object_key: str) -> dict[str, str]:
    ...
    if settings.demo_mode or not settings.google_drive_ready:
        ...
        repository.append_activity(
            f"Demo processing completed for {project.name}",
            kind="processing",
        )
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="shortsmith-") as temp_dir:
            ...
            for index in range(num_chunks):
                ...
                raw_chunk = processor.split_fixed_chunk(
                    ...
                    duration_seconds=max(1, int(actual_duration)),
                )
                ...
                if transcriber.ready:
                    audio_path = processor.extract_audio(
                        reframed,
                        temp_path / f"chunk-{chunk_num:02d}.mp3",
                    )
                ...
                thumbnail = processor.extract_thumbnail(
                    subtitled,
                    temp_path / f"chunk-{chunk_num:02d}.jpg",
                    timestamp=1.0,
                )
        ...
        return {"status": "completed", "chunks": str(num_chunks)}
    except Exception as exc:
        project.status = ProjectStatus.FAILED
        project.processing_stage = "Processing failed"
        project.queue_depth = 0
        project.updated_at = datetime.now(timezone.utc).isoformat()
        repository.save_project(project)
        repository.append_activity(
            f"Processing failed for {project.name}: {exc}",
            kind="processing",
        )
        raise
